...
vector<char> ma_star_search(char start_state, char goal_state, function<int(char, char)> heuristic, int memory_limit) {
    ...
    while (!open_list.empty()) {
        ...
        if (closed_set.size() > memory_limit) {
            return vector<char>();
        }
...
int main() {
    ...
    vector<char> path = ma_star_search(start_state, goal_state, heuristic, memory_limit);
    cout << "MA* Path: ";
    for (char state : path) {
        cout << state << " ";
    }
