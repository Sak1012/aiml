...
char* ma_star_search(char start_state, char goal_state, int memory_limit) {
    ...
    while (open_list_size > 0) {
        ...
        if (closed_set_size > memory_limit) {
            return NULL;
        }
...
int main() {
    ...
    char* path = ma_star_search(start_state, goal_state, memory_limit);
    printf("MA* Path: %s\n", path);
    free(path);
    return 0;
}
